...
/**
 * Memoized Sidebar Container Component
 * Requirements: 8.1, 8.2
 * 
 * This component is wrapped with React.memo to prevent re-renders
 * when chat state updates. It is isolated from chat component state.
 */
...
const SidebarContainerComponent: React.FC<SidebarContainerProps> = ({ data, thought, onTopicTrigger }) => {
  // Isolated state - changes here won't affect chat component (Requirements: 8.2)
  ...
  // Memoized callbacks
  const handleIntakeTabClick = useCallback(() => setActiveTab('intake'), []);
  const handleHandoverTabClick = useCallback(() => setActiveTab('handover'), []);
  const handleToggleUCG = useCallback(() => setShowUCG(prev => !prev), []);
  const handleToggleFullAnalysis = useCallback(() => setShowFullAnalysis(prev => !prev), []);
    ...
    <div className="h-full flex flex-col bg-white border-l border-slate-200">
      <div className="p-4 border-b border-slate-100 space-y-4">
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg">
          ...
          <button
            onClick={handleHandoverTabClick}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50/50">
        {activeTab === 'intake' ? (
          <IntakeTab data={data} thought={thought} onTopicTrigger={onTopicTrigger} />
        ) : (
          <HandoverTab
            data={data}
            showUCG={showUCG}
            onToggleUCG={handleToggleUCG}
...
const IntakeTab = React.memo<IntakeTabProps>(({ data, thought, onTopicTrigger }) => (
  <>
              ...
              thought.differentialDiagnosis.map((ddx, i) => (
                <div key={i} className="flex flex-col bg-slate-50 p-2 rounded-lg border border-slate-100">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${ddx.probability.toLowerCase().includes('high') ? 'bg-red-100 text-red-700' :
...
IntakeTab.displayName = 'IntakeTab';

// Memoized HandoverTab component
...
const HandoverTab = React.memo<HandoverTabProps>(({ data, showUCG, onToggleUCG, showFullAnalysis, onToggleFullAnalysis }) => (
  <div className="space-y-6">
    <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <p className="text-blue-100 text-sm">
          Generated by HelloDoctor (Senior Consultant AI)
        </p>
      </div>
    </div>
    ...
    <div className="bg-purple-50 border border-purple-100 rounded-xl shadow-sm overflow-hidden">
      <button
        ...
        className={`w-full flex items-center justify-between p-4 transition-all ${showUCG ? 'border-b border-purple-100 bg-purple-100/50' : 'hover:bg-purple-100/30'
...
HandoverTab.displayName = 'HandoverTab';

// Memoized SBARCard component
...
const SBARCard = React.memo<SBARCardProps>(({ title, icon, content, colorTheme }) => {
  const styles = {
    blue: {
...
// Memoized RecordSection component
...
const RecordSection = React.memo<RecordSectionProps>(({
...
RecordSection.displayName = 'RecordSection';

/**
 * Custom comparison function for React.memo
 * Only re-renders if data or thought changes (Requirements: 8.1)
 */
const arePropsEqual = (
  prevProps: SidebarContainerProps,
  nextProps: SidebarContainerProps
): boolean => {
  // Deep comparison for data and thought would be expensive,
  // so we rely on reference equality which works with immutable updates
  return (
    prevProps.data === nextProps.data &&
    prevProps.thought === nextProps.thought &&
    prevProps.onTopicTrigger === nextProps.onTopicTrigger
  );
};

export const SidebarContainer = React.memo(SidebarContainerComponent, arePropsEqual);
